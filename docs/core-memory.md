...
| Session continuation (M5) | `Session/SessionSnapshot.swift`, `cch-mcp/SessionSummarizer.swift`, `Memory/LastSessionCard.swift` | Hook spawns a detached `cch-mcp snapshot`; condensed transcript → background model (`--tools ""`) → one `session` memory per Claude session, linked `touched_in` to retrieved/cited memories. The **Pick up from last session** card surfaces that summary (Next steps/Open threads/Goal/Done) to the user above the main terminal — Claude gets it via the retrieval hook. |
