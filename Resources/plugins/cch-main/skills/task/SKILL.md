---
name: task
description: Spawn a Task subagent (scoped change) in its own git worktree
argument-hint: <what to do>
---
The user wants a TASK subagent for: $ARGUMENTS

1. Call mcp__plugin_cch-main_cch__list_subagents. Note each active subagent's title, status and changed_files.
2. Write a self-contained brief for the subagent (a scoped change; follow the instructions exactly): goal, relevant context from this conversation and core memory, likely files, acceptance criteria, constraints, and how to verify.
...
5. Call mcp__plugin_cch-main_cch__spawn_subagent(category: "task", title: <at most 6 words>, brief: …).
